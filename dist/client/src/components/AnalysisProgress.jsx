...
export default function AnalysisProgress({ fileName, onComplete, isAnalyzing = false }) {
    ...
    <p className="text-sm text-muted-foreground text-center animate-pulse">
      Running TF-IDF • Cosine Similarity • Image Intelligence
        ...
        <div className="space-y-4">
          {steps.map((step, index) => (<div key={step.id} className={`flex items-center gap-3 p-3 rounded-lg transition-colors ${index === currentStep ? 'bg-primary/5 border border-primary/20' :
                step.completed ? 'bg-green-50 border border-green-200' :
                    'bg-muted/30'}`} data-testid={`step-${step.id}`}>
              <div className={`
              ...
              <div className="flex-1 min-w-0">
                ...
                index === currentStep ? 'text-primary' :
                  ...
                  {(index === currentStep || step.completed) && (<span className="text-sm font-medium" data-testid={`progress-${step.id}`}>
                      {step.progress}%
                    </span>)}
                </div>
                ...
                {index === currentStep && !step.completed && (<Progress value={step.progress} className="h-1 mt-2"/>)}
              </div>
            </div>))}
        </div>
      </CardContent>
