...
export default function TextComparison({ originalText, sourceText, sourceName, sourceType, overallSimilarity, onBack, }) {
    ...
    const renderHighlightedText = (textArray = [], testId) => (<div className="space-y-2" data-testid={testId}>
      {textArray.map((segment, index) => (<span key={index} className={segment.isHighlighted
                ? getSimilarityColor(segment.similarityLevel)
                : ""}>
          {segment.text}
        </span>))}
    </div>);
    return (<div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Button variant="outline" size="sm" onClick={onBack} data-testid="button-back-to-report">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Comparing with:{" "}
              <span data-testid="text-source-name">{sourceName}</span>
            </p>
          </div>
        </div>
        ...
        <TabsContent value="inline" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <p className="text-sm text-muted-foreground">
                Highlighted sections show similarities between your document and
                the source.
              </p>
            </CardHeader>
            <CardContent>
