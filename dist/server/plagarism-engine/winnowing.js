 ...
 */
export function winnow(text, k = 5, w = 4) {
    const normalized = normalizeText(text);
    const fingerprints = new Set();
    const fingerprintPositions = new Map();
 ...
 *   similarity = |A ∩ B| / |A ∪ B|
 */
export function compareWithWinnowing(textA, textB, k = 5, w = 4) {
 ...
 */
export function findMatchedPassages(textA, textB, k = 5, w = 4) {
    ...
    for (const p of passages) {
        if (merged.length && p.start <= merged[merged.length - 1].end) {
            ...
            merged[merged.length - 1].text = textA.slice(merged[merged.length - 1].start, merged[merged.length - 1].end);
        }
        else {
            merged.push({ ...p });
        }
