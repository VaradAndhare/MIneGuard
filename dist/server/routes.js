...
export async function registerRoutes(app) {
    ...
    // ==================================================================
    app.post("/api/image-check", upload.single("file"), async (req, res) => {
        try {
            if (!req.file)
    ...
    app.post("/api/analyze", upload.single("file"), async (req, res) => {
        try {
            ...
            const userId = req.userId; // set by auth middleware if you add it
            const result = await analyzeDocument(text, req.file.originalname, userId);
            console.log(`🔍 Analysis complete: ${result.similarity}% similarity | ` +
    ...
    // ==================================================================
    app.get("/api/compare/:docId", authMiddleware, async (req, res) => {
        try {
    ...
    app.post("/api/compare/:docId", async (req, res) => {
        try {
            const { docId } = req.params;
            const { uploadedText } = req.body;
            if (!uploadedText) {
